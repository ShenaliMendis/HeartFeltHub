...
try
{
	...
	ResultSet rs2=st.executeQuery("select *from users inner join cart where.email='"+email ""' and cart.status='bill'");
	while(rs2.next())
	{
%>
...
<h3>Product Details</h3>
  ...
  <%
  ResultSet rs1=st.executeQuery("select *from cart inner product where cart.product_id=product.id and cart.email='"+email+"' and cart.status='bill'");
   while(rs1.next())
   {
   	sno=sno+1;
   %>
